...
void monte_carlo_tree_search(unsigned int tid, Node* root, std::chrono::_V2::system_clock::time_point start_time);
...
int main(int argc, char** argv) {
    ...
    for (int t = 0; t < num_threads - 1; t++) {
        threads[t] = std::thread(monte_carlo_tree_search, t, root, start_time);
    }
    monte_carlo_tree_search(num_threads - 1, root, start_time);
    for (int t = 0; t < num_threads - 1; t++) {
        threads[t].join();
    }
...
void monte_carlo_tree_search(unsigned int tid, Node* root, std::chrono::_V2::system_clock::time_point start_time) {
    ...
    while (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - start_time).count() < MAX_DURATION) {
        iteration++;
        traversal(tid, root, curnode);
        if (!isTerminal(curnode) && curnode->partial_games != 0) {
            curnode->lock_children.lock();
            expansion(curnode);
            curnode->lock_children.unlock();
            curnode = curnode->children[std::min((size_t)tid, curnode->children.size() - 1)].first;
        }
        win = 0;
